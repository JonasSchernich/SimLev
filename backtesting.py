...
# ---------------------------------------------------
# Datenvorbereitung: Input DataFrame erstellen
# ---------------------------------------------------
spx = pd.read_csv(
    "data/SPX.csv",
    sep=";",
...
# Optional: DataFrame zuschneiden
kgv_on_spx_days = kgv_on_spx_days.iloc[10000:].reset_index(drop=True)
...
def run_backtest(
    ...
    rbc_r: float,
    rbc_sigma: float,
    rbc_maturity_in_years: float,  # Wird initial übergeben, aber wir überschreiben dynamisch
    initial_capital: float,
    plot: bool,
    tax_rate: float,  # als Bruch, z.B. 0.25 für 25%
    # Für Bull Case:
    bull_metric: str,          # "kgv", "preis" oder "tage"
    bull_thresholds: list,     # Liste der Schwellenwerte (R1->R2, R2->R3, …)
    bull_allocations: list,    # Liste von Allokationen (Reihenfolge: [1x, 2x, 3x, fix, Bonus])
    # Für Bear Case:
    bear_thresholds: list,     # Liste der Drawdown-Schwellenwerte
    bear_allocations: list     # Liste von Allokationen (Reihenfolge: [1x, 2x, 3x, fix, Reverse Bonus])
):
    ...
    # Für den Bear Case (Reverse Zertifikat) verwenden wir:
    current_T_bear = 1.5  # Startwert in Jahren
    rollover_bear = False

    # Für Bull Case (Bonus Zertifikat) – belassen wir den bisherigen Ansatz (hier nicht angepasst)
    current_T_bull = 1.0
    ...
    # Hilfsvariablen
    last_bull_pe = None         # für KGV im Bull
    ...
    last_ath_price = None       # Letzter ATH aus der Bull-Phase

    # Regime-Status speichern (für den Plot)
    ...
    for i in range(1, len(df)):
        ...
        if row["RegimeChangeFlag"]:
            if regime_active == "Bear":  # Wechsel Bull->Bear
                ...
                bull_regime_index = 1
                current_T_bull = 1.0  # Reset Bull T (wie gehabt)
            else:  # Wechsel Bear->Bull
                last_bull_pe = row[pe_col]
                last_bear_low = None
                last_bear_low_date = None
                bull_regime_index = 1
                current_T_bull = 1.0
            bear_regime_index = 1
            current_T_bear = 1.5  # Reset Bear T bei Regimewechsel
            rollover_bear = False
            rebalance_count += 1
        ...
        if regime_active == "Bull":
            # Bull Case: (Bonus Zertifikat) – hier belassen wir die bestehende Logik
            T_cert = current_T_bull
            if len(bull_allocations) > 1:
                if bull_metric.lower() == "kgv":
                    if last_bull_pe is None:
                        last_bull_pe = df.loc[i-1, pe_col]
                    metric_value = (row[pe_col] - last_bull_pe) / last_bull_pe
                elif bull_metric.lower() == "preis":
                ...
                if bull_regime_index < len(bull_allocations):
                    if metric_value >= bull_thresholds[bull_regime_index - 1]:
                        ...
                        rebalancing = True
                        rebalance_count += 1
                        current_T_bull = 1.0  # Reset T beim Wechsel
                else:
                    bull_regime_index = 1
            else:
                bull_regime_index = 1
            ...
            # Hier belassen wir die Bull-Logik wie gehabt
            ...
            # Bear Case (Reverse Zertifikat) – hier wird die dynamische Restlaufzeit angepasst:
            ...
            if len(bear_allocations) > 1:
                if bear_regime_index < len(bear_allocations):
                    if drawdown >= bear_thresholds[bear_regime_index - 1]:
                        ...
                        rebalancing = True
                        rebalance_count += 1
                        current_T_bear = 1.5  # Reset T beim Wechsel
                        rollover_bear = False
                else:
                    bear_regime_index = 1
            else:
            ...
            # Aktualisiere die Restlaufzeit im Bear Case:
            current_T_bear -= 1/365
            # Falls T unter 1.4 fällt, soll ein Reset erfolgen (RollOver), aber in diesem Fall
            # soll **keine Steueranpassung** erfolgen (da es nur der Zeitablauf ist).
            time_rollover_event_bear = False
            if current_T_bear < 1.4:
                current_T_bear = 1.5
                rebalancing = True
                # Kennzeichne, dass dies ein reiner Zeitrollover ist, bei dem keine Steuern gelten.
        ...
        if row["RegimeChangeFlag"] or rebalancing:
            ...
            # Bei Bear Rollovers aufgrund von Zeitablauf (kein Regimewechsel) keine Steuern:
    ...
    # Plot: Portfolioverlauf mit farbiger Markierung der Regimeabschnitte
...
def get_user_parameters():
    ...
    if not indicator_type: indicator_type = "SMA"
    ...
    if bull_regime_count > 1:
        bull_metric = input("Welche Metrik soll für den Wechsel zu Bull Regimen verwendet werden? (KGV/Preis/Tage) [Standard: KGV]: ")
        if not bull_metric: bull_metric = "kgv"
    else:
        bull_metric = None
...
#368029
# base mit 250days:576222
# base mit 0.98:649893
## base mit 0.98: mit 0.4 buy the dip1.551703e+06
#base mit 0.98: + bonus zert:1,174,064,000
#1,075,868
#Base Rendite:17,5% vs27%im paoer->2/3 der TR Rendite
